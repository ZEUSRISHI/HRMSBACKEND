...
const createUser = async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      phone, countryCode, dob, department, designation,
      address, joiningDate, gender,
      emergencyContact, emergencyCountryCode, avatar,
    });
...
const updateUser = async (req, res) => {
  try {
    const allowed = [
      "name", "email", "role", "phone", "countryCode", "dob",
      "department", "designation", "address",
      "joiningDate", "gender", "emergencyContact",
      "emergencyCountryCode", "avatar", "isActive",
    ];
    ...
    if (req.body.emergencyContact && !PHONE_REGEX.test(req.body.emergencyContact)) {
      return res.status(400).json({
        success: false,
        message: "Emergency contact must contain only digits (4-15 characters).",
      });
