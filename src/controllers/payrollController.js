...
function generatePayslipPDF({
  ...
  return new Promise((resolve, reject) => {
    ...
    /* ════════ HEADER — starts lower for top margin ════════ */
    const logoSize = 48;
    const logoX    = margin;
    const logoY    = 40; // increased top space
    ...
    doc.moveTo(margin, dividerY).lineTo(pW - margin, dividerY)
    ...
    // Extra gap row after Conveyance Allowance before Gross Earnings
    const CONVEYANCE_EXTRA_ROWS = 3; // 3 blank rows ≈ 3-4 lines gap
    ...
    // Add blank rows for the gap after conveyance allowance
    const extraBlankRows = CONVEYANCE_EXTRA_ROWS;
    const numDataRows = Math.max(earningsRows.length, deductionRows.length);
    const numRows     = numDataRows + extraBlankRows; // total rows including gap
    ...
    // Draw data rows (earnings + deductions side by side)
    ...
    // Draw extra blank rows (gap between conveyance and gross earnings)
    for (let i = 0; i < extraBlankRows; i++) {
      // light alternating shade for blank rows
      if ((numDataRows + i) % 2 === 0) doc.rect(tX, rowY, tW, rH).fill("#fafafa");
      rowY += rH;
      doc.moveTo(tX, rowY).lineTo(tX + tW, rowY)
         .strokeColor("#f1f5f9").lineWidth(0.3).stroke();
    }
    ...
    /* ════════ DIVIDER ABOVE SYSTEM-GENERATED TEXT ════════ */
...
const footerY = doc.page.height - 45; // Distance from bottom

// Divider above footer
doc.moveTo(margin, footerY - 12)
   .lineTo(pW - margin, footerY - 12)
   .strokeColor("#e2e8f0")
   .lineWidth(0.8)
   .stroke();
...
const totalWidth = footerLogoSize + 8 + textWidth;
...
/* ── NEW: viewPayslip — streams PDF inline to browser ── */
const viewPayslip = async (req, res) => {
  try {
    ...
    const buf = await generatePayslipPDF({
      name:        payroll.userId?.name        || "Employee",
      employeeId:  payroll.employeeId          || payroll.userId?.employeeId || String(payroll.userId?._id),
      role:        payroll.role,
      month:       payroll.month,
    ...
    const safeName = (payroll.userId?.name || "Payslip").replace(/\s+/g, "_");
    res.setHeader("Content-Type",        "application/pdf");
    res.setHeader("Content-Disposition", `inline; filename="Payslip_${safeName}_${payroll.month}.pdf"`);
...
module.exports = {
  ...
  markAsPaid, resendPayslip, viewPayslip,
